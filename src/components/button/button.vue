<template>
	<button :class="['button', width, 'theme-' + theme]">
		<slot></slot>
	</button>
</template>
...
interface Props {
	width?: "normal" | "full";
	theme?: "outline" | "solid" | "text";
}

const { width = "normal", theme = "outline" } = defineProps<Props>();
</script>

<style scoped>
.button {
	--button-padding: 0.5em 1em;
	--button-col-gap: 0.5em;
...
.theme-dark .button.theme-solid {
	--button-hover-font-color: var(--main-font-color);
}
.theme-dark .button.theme-text {
	--button-font-color: var(--main-font-color);
	--button-hover-font-color: var(--main-font-color);
}
